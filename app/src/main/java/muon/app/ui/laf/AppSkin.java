...
public abstract class AppSkin {
	...
	protected Font loadFonts() {
		...
		try (InputStream is = AppSkin.class.getResourceAsStream("/fonts/SarasaMonoSC-Regular.ttf")) {
			if (is != null) {
				Font cjkFont = Font.createFont(Font.TRUETYPE_FONT, is);
				ge.registerFont(cjkFont);
			}
		} catch (Exception e) {
			e.printStackTrace();
		}
		...
		Font uiFont = new Font("Sarasa Mono SC", Font.PLAIN, 12);
		if (!"Sarasa Mono SC".equalsIgnoreCase(uiFont.getFamily())) {
			uiFont = new Font(Font.DIALOG, Font.PLAIN, 12);
		}
		return uiFont;
	}
